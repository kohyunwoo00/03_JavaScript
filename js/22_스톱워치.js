...
// START 버튼 클릭 시 수행되는 setInterval() 저장변수
let currentInterval;
...
startBtn.addEventListener("click", ()=>{
  ...
  if(startBtn.innerText === "PAUSE"){
    ...
    // setInterval()을 깨끗이 치워버림 -> clearInterval() 이용
...
resetBtn.addEventListener("click", () => {
  startBtn.innerText = "START";  //START 버튼 형태로 변겅
  recordBtn.disabled = true;
  count = 0; 
  clearInterval(currentInterval); // 반복 종료
  recordContainer.innerHTML = "" // 내용 지우기

  // 화면 숫자 00:00:00 함수 호출()
  displayOutput();
});
// ---------------------------------------------------
 ...
 * @param num : 숫자
 ...
 function displayOutput(){
  ...
  const second = attachZero(Math.floor(count/100 % 60));  // floor -> 몫 구할때 사용
...
/* RECORD 버튼 클릭 시
  - #display에 작성된 내용을 모두 읽어와
    <li>읽어온 시간</li> 형태를 만들어 #recordContainer의 마지막 자식으로 추가 */
recordBtn.addEventListener("click", () => {
  
 // console.log(display.innerText);
  // console.log(display.innerHTML); -> 태그까지 모두 얻어옴(복잡함)
  /* li 태그 생성 */
...
/* clearInterval() 사용 방법
  ...
  1. setInterval() 수행 후 반횐 되는 값을 변수에 대입
   ex) const currentInterval = setInterval();
  2. clearInterval(변수명) 수행
   es) clearInterval(currentInterval);
     // -> setInterval() 동작 멈춤
*/