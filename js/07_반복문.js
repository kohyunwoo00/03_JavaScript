...
/* 다음 모양 출력하기 */
function check17(){
  for(let row =1; row <= 3; row++){  //3번 출력(3줄)
...
/* 다음 모양 출력하기 */
function check18(){
  for(let row =1; row <= 4; row++){  //4번 출력(4줄)
    let result = ""; // 빈칸(string)
    for(let col=1; col <=5; col++){ // 12345
      result += col; // 문자열 + 숫자 -> 이어쓰기
    }
    console.log(result);
    console.log("-----"); 
...
function check21(){
 ...
 입력 : 5
      ...
      ***** */
 function check23(){
  const a = document.getElementById("input23");
  if(a.value.length === 0){  // 미입력 시
    alert("숫자를 입력해주세요");
    return;
  }
...
function check29(){
  ...
  // undefined랑 null은 다름!!)
  while(input !== null){
    //prompt에 반환되는 값 종류 : "1", "2", "3", 메뉴없는 숫자, null(취소)
    input = prompt("메뉴 번호 입력(1. 김밥 2. 라면 3. 돈까스)")

    switch(input){
      case "1" : gCount++; break;
      case "2" : rCount++; break;
      case "3" : dCoungt++; break;
