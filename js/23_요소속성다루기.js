...
btn1.addEventListener("click", ()=>{
  ...
  // JS 속성 설정 방법 중 class 관련 + CSS class 기반 설계를 이용해서
  // HTML 요소의 스타일을 변경 할 수 있다.
...
btn2.addEventListener("click", ()=>{
  // [tip] 브라우저 콘솔에서 특정 요소가 가지고있는 DOM 속성을
  // 직접 확인하고 싶을 경우 -> 복수 형태로 요소를 얻어오는
  ...
  console.log(inputs[0].id);  // 위에랑 똑같은 결과
  ...
  console.log("#div2의 name", divs[0].value);
});
...
addBtn.addEventListener("click", ()=>{
  ...
  // .btn-container 에 hidden 클래스가 포함되어 있을 경우
  if(btnContainer.classList.contains("hidden")){
    //요소.clasList.remvoe("클래스1" [,...]) -> 클래스 제거
    btnContainer.classList.remove("hidden");


    addBtn.classList.add("hidden"); // 클래스 추가
  }
});
...
closeBtn.addEventListener("click", () => {
  ...
  if( !btnContainer.classList.contains("hidden")){
    ...
    addBtn.classList.remove("hidden");  // hidden제거
  }
});

/*
- 클래스가 이용한 CSS 스타일 제어하는 방법이 제일 많이 사용됨
-> HTML 복잡도가 낮아짐
...
- 요소.style.css 속성 방법은 inline-style로 적용되기 때문에 
  ...
  * 왜? css 선택자 우선순위 참고(1에 가까울 수록 우선)
  1) !important
  2) inline-style
  3) id 선택자
  4) class 선택자
...
btns.forEach((btn)=>{
  ...
  btn.addEventListener("click", ()=>{
    const color = btn.innerText; // 버튼에 작성된 내용 얻어옴(색상)
    ...
    if(!classExist){
    container3.classList.toggle(`bg-${color}`); // add 가능
    }
  });
});
