...
function check1(){
  ...
  if(val > 0){
    ...
    alert(`${val} 은 양수 입니다.`); // alert : 브라우저 알림창 띄우기
  }
...
function check2(){
  ...
  // 1 ~ 100 사이의 난수를발생시켜 randomNumber 변수에 저장
...
/* input 요소에 작성된 값 얻어오기 : input요소.value, html 요소에 작성된 내용 얻어오기 : html요소.innerText */
...
function check4(){
  ...
  if(inputA.value.length === 0 || inputB.value.length === 0){
    ...
    return; // 함수를 즉시 종료하고 호출한 곳으로 돌아감 -> 함수를 수행할 수 없는 상태여서 미리 종료
  }
  ...
  // 위 if문을 건너 뜀 === A,B가 모두 입력됨 + A가 B보다 크거나 같음
...
function check6(){
  ...
  // -> break 미작성 시 현재 case 수행 후 멈추지 않고 다음 case를 연달아서 수행 
  let result;
  switch(Number(input6.value)){
    // case3,4,5 : 자바만 사용 가능 (JS 안됨)
