/* JS를 이용해서 CSS 변경하는 방법
  ...
  * 요소.style.css속성 = 값
  -> inline 스타일로 CSS가 추가됨(시작태그에 style 속성이 추가됨)*/
...
function classTest2(){
  ...
  for(let i = 0; i<inputs.length; i++){
    // 각 요소(iniput)에 작성된 값을 얻어와 number 타입으로 변환하여 sum에 누적
...
function cssTest(){
  ...
  for(let i=0; i<divs.length; i++){
    ...
    divs[i].style.justfyContent = "center"; // 중심축 방향 가운데
...
function readValue(){
  ...
  // 요소.scrollTop =  스크롤 제일 위에서 부터 현재 위치
  // 요소.scrollTop = "값"
