...
// 전달 인자(Argument) : 함수 호출 시 () 안에 작성하는 값 / 함수 매개변수로 전달됨
// 매개 변수(Parameter) : 함수 정의 시()에 작성하는 변수, 호출 시 전달된 값을 저장
...
function check1(div){
  ...
  // 3) 얻어온 내용이 숫자인 경우
  div.innerText = num + 1; // 1씪 증가 ++num 으로 쓸거면 const -> let으로 바뀌어야함
}
...
testA.onclick = function(){
  ...
  /*  거전 이벤트 모델에서 this == 이벤트가 발생하는 요소 */
  console.log(this);
  alert("고전 이벤트 모델 확인됨");
};

/* 고전 이벤트 모델 제거 */
// #testB 버튼 클릭 시 #testA의 이벤트 핸들러 제거
...
test1.addEventListener("click", function(){
  ...
  // 0 : 투명 / 1 : 불투명(선명하게 보임)
  let current = test1.style.opacity; // opacity : 불투명
...
/* "변경하기" 버튼 클릭 시  input 요소에 작성된 색상 값을 얻어와 
  ...
  const v1 = document.querySelectorAll(".inputColor"); // 여러개니까 All 아니면 빼기, input 5개
  const dps = document.querySelectorAll(".display");
  // 변경하기 버튼
  const btn = document.querySelector("#changeBtn");
  // 변경하기 버튼이 클릭 되었을 때
  btn.addEventListener("click",function(){
    for(let i=0; i<dps.length; i++){
      // i번째 .inputColor에 작성된 값을 얻어와 i번째 .display의 배경색으로 대입
      dps[i].style.backgroundColor = v1[i].value;
    }   
  });
