...
btn2.addEventListener("click", ()=>{
  ...
  student.introduce = function(){
    ...
    alert(`저는 ${this.age}세 ${gender} ${this.name} 입니다`);
      // 메서드 내에서 this === 메서드가 작성된 객체(여기서는 student)
  }
    student.introduce();
...
/* this 참조 변수 위치에 따른 의미 
   ...
   2) <button onclick = "testFn(this)" > 테스트</button>
      ...
      btn.addEveentListener("click", function(){
      console.log(this);
  });
...
function Student(grade, ban, number, name){
  ...
  // 생성딘 Student 객체의 key가 grade인 필드에 value로 전달 받은 grade를 대입
  this.grade = grade;
  this.ban = ban;
  this.number = number;
  this.name = name;
...
// --------------------------------------------------------------------------------------------
/* 학생 조회 버튼클릭시
  studentList에 저장된 모든 내용을 #tbody에 요소를 만들어서 출력 */
...
checkJson.addEventListener("click", () => {
  ...
  const menu = '{"name" : "김밥", "price" : 4000}'; // {} JS의 객체
