...
// 함수 내부에 작성되지 않은 코드 -> HTML 페이지 로딩(랜더링)시 script 파일이 전체 해석되면서 같이 해석/수행 된다

// 잔액 초기 값 출력
output.innerText = balance;

// 금액 입력 input 태그의 value 값을 ""(빈칸)으로 변경
amount.value = ""; // input에 작성된 값을 삭제하는 효과 -> 입, 출금 함수 마지막에 사용하기
...
function test(){
  ...
  // 입력 받은 비밀번호(pw)와 전역 변수 password가 다를 경우 == 비밀번호 불일치
  if(pw != password){
