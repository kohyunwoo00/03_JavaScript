...
function check5(){
  ...
  // 1) continue 사용 x
  // if(num % 3 !== 0 && num % 5 !==0){ // 3,5 배수가 아닐때
  //   console.log(num);
  // }  
}
...
function check6(){
  ...
  for(let num =1; num <=5; num++){
    let str ="123456789";
    str = str.replace(num, ""); // num과 일치하는 숫자를 str에서 찾아 ''으로 변경
    console.log(str);
  }
}
...
/* UP & DOWN 게임 */
function startGame(){
  // Math.floor(실수); -> 소수점 내림 처림(정수로 변함)/소수점 없앰 
  // 맞춰야되는 정답(1 ~ 200 사이 난수) 생성
  // Math.random() -> 0.0 ~ 1.0 사이 난수 발생(1.0 미만)
  const answer = Math.floor(Math.random() * 200 + 1);  // 1 ~ 200
