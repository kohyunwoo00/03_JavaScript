...
function check1(){
 ...
 // 배열 길이 확인
  ...
  // 배열 요소에 값 대입 : 배열명[inex] = 값;
  arr2[0] = 123;
  arr2[1] = "27일은 임시공휴일";
  arr2[2] = true;
  // (여러 자료형 저장 가능 확인)

  // * 존재하지 안흔ㄴ index에 값 대입 -> 가능! + 배열 길이가 자동으로 증가함
  arr2[3] = "가능?"; // 3번 인덱스에 대입됨(확인 완)
  arr2[6] = "건너뛰기도 가능?"; 
  // 6번 인덱스 대입됨(확인 완) + 건너뛴 4, 5번 인덱스가 비어있음 상태가 되어있음
...
function check4(){
  ...
  // 1) prompt를 이용해 숫자 5개를 입력 받아 배열에 저장한 후
  for(let i=0; i<5; i++){
...
/* 점수 메뉴 뽑기 */
function selectMenu(){
  const menus = ["알탕", "칼국수", "햄버거", "돌솥비빔밥", "불뚝","라면", "김밥", "주먹밥", "돈까쓰", "순두부"];
  // 0이상 메뉴 개수 미만의 난수 1개생성
...
/* 4행 4열 짜리 2차원 배열에 1 ~ 16 사이 난수 배치(중복 X) 
    1) 콘솔에 출력 
    2) promp를 이용해서 1 ~ 16 사이 숫자를 입력 받아 
...
function check7(){
   ...
   for(let i = 0; i< 16; i++){
    const num = Math.floor(Math.random() * 16 + 1); // 1 ~ 16의 난수
  ...
  for(let row = 0; row<4; row++){ // 행 반복
    ...
    index++; // index를 1 증가 시켜 randdomArr 다음 요소 지정하기
    }
  }
  console.log(arr);
