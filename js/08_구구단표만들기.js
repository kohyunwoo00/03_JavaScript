/* 구구단 표 만들기 */
function createTable(){
  /* 시작, 종료 단 input 요소 얻어오기 */
  const startInput = document.getElementById("start");
  const endInput = document.getElementById("end");

  /* 테이블 머리(단 입력  영역) */
  const thead = document.getElementById("thead");

  /* 테이블 몸통(구구단 표 출력 영역) */
  const tbody = document.getElementById("tbody");

  // -----------------------------------------------
  // 기존에 남아있던 표를 삭제하는 작업/ inner(시작문구와 종료 문구 사이 내용에 들어갈 내용)
  thead.innerHTML = ""; // 시작/종료 태그 사이 내용을 빈칸으로 바꿈 -> 삭제 효과
  tbody.innerHTML = "";

  // 시작 또는 종료 단이 입력되지 않은 경우
  // -> alert("시작/종료 단을 입력 해주세요")

  // 시작 또는 종료 단에 0이하가 입력된 경우
  // -> alert("1 이상의 수만 입력 해주세요");


  // 시작 단이 종료 단 보다 큰 경우
  // -> alert("시작 단이 종료 단 보다 클 수없습니다");

  // ------------------------------------------------
  /*  구구단 표 만들기 코드 작성 */
  const a = Number(startInput.value)
  const b = Number(endInput.value)
  
  if(startInput.value.length === 0 || endInput.value.length === 0){
    alert("시작/종료 단을 입력 해주세요");
    return;
  }
  if(a <= 0 || b <= 0){
    alert("1 이상의 수만 입력 해주세요");
    return;
  }
  if(a > b){
    alert("시작 단이 종료 단 보다 클 수 없습니다");
    return;
  }
  // #thead에 들어갈 html 코드 생성
  let output1 = "<tr>";
  for(let num = a; num <= b; num++){  //<th></th> 생성
    output1 += `<th>${num}단</th>`;
  }
  output1 += "</tr>";

   // 생성한 html 코드를 HTML 문서에 추가 -> 클릭
   thead.innerHTML = output1;

  // 구구단 표 몸통 부분
  // 곱해지는 수 (1~9) + tr태그의 개수
  for(let num = 1; num <= 9; num++){
    let output2 = "<tr>";
    // <td>단 x num = 결과</td>
    for(let dan = a; dan <= b; dan++){
      output2 +=`<td>${dan} x ${num} = ${dan * num}</td>`;
    }
    output2 +="</tr>";
    tbody.innerHTML += output2;
    // for end -> tr 한 줄 생성
    // --> 화면에 추가
  }
  

 
   
}